...
<body class="bg-[#f8fafc] text-slate-900">
    ...
    <header class="relative min-h-screen flex items-center islamic-bg text-white overflow-hidden">
        ...
        <div class="container mx-auto px-6 relative z-10 text-center">
            ...
            <div class="flex flex-wrap justify-center gap-6">
                <button onclick="generateInsight(this)" class="magic-button text-temurid-900 px-10 py-5 rounded-2xl font-bold hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center gap-3 min-w-[220px] justify-center">
                ...
                <button onclick="speakText('Kuch adolatdadir. Adolat esa har bir ishning asosidir.', this)" class="bg-white/10 backdrop-blur-xl border border-white/20 px-10 py-5 rounded-2xl font-bold hover:bg-white/20 transition-all flex items-center gap-3">
                    <i class="fa-solid fa-microphone-lines"></i> Salobatli ovozda tinglash
                </button>
            </div>
    ...
    <main class="py-24 container mx-auto px-6 space-y-32">
        ...
        <section class="max-w-4xl mx-auto text-center space-y-8">
            <div class="inline-block p-4 bg-temurid-gold/10 rounded-full mb-4">
                <i class="fa-solid fa-scroll text-temurid-gold text-3xl"></i>
            </div>
            <h2 class="serif text-5xl text-temurid-900">✨ Tuzuklar asosida tavsiya</h2>
            <p class="text-xl text-slate-600">Zamonaviy muammongizni yozing, Sohibqiron "Tuzuklar" asosida sizga yo'l ko'rsatadi.</p>
            <div class="flex flex-col gap-4">
                <textarea id="advice-input" placeholder="Masalan: Jamoani qanday boshqarish kerak? yoki Murakkab qaror qabul qilishda nima muhim?" 
                    class="w-full p-6 rounded-3xl border-2 border-slate-200 focus:border-temurid-gold outline-none text-lg resize-none h-32 transition-all"></textarea>
                <button onclick="getTemurAdvice(this)" class="magic-button text-temurid-900 py-5 rounded-2xl font-black text-lg uppercase tracking-widest flex items-center justify-center gap-3">
                    <i class="fa-solid fa-brain"></i> <span>✨ Maslahat olish</span>
                </button>
            </div>
            <div id="advice-result" class="hidden glass p-10 rounded-[3rem] text-left border-l-8 border-temurid-gold animate-fade-in">
                <h4 class="text-temurid-gold font-bold mb-4 flex items-center gap-2">Sohibqiron o'giti:</h4>
                <p id="advice-text" class="text-xl text-slate-100 italic leading-relaxed"></p>
                <button onclick="speakText(document.getElementById('advice-text').innerText, this)" class="mt-6 text-temurid-gold flex items-center gap-2 hover:underline">
        ...
        <section class="max-w-5xl mx-auto" id="muloqot">
            <div class="glass p-12 rounded-[3.5rem] shadow-2xl border-2 border-temurid-gold/20 relative overflow-hidden islamic-bg">
                ...
                <div class="flex flex-col md:flex-row gap-4 relative z-10">
                    ...
                    <button id="send-btn" onclick="askAI()" class="magic-button text-temurid-900 px-12 py-5 rounded-2xl font-black hover:opacity-90 active:scale-95 shadow-2xl transition-all uppercase tracking-wider flex items-center justify-center gap-2">
    ...
    <script>
        ...
        async function speakText(text, btn) {
            ...
            try {
                const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key=${apiKey}`, {
                    ...
                    body: JSON.stringify({
                        contents: [{ parts: [{ text: `Vazmin, juda yo'g'on, haybatli va salobatli hukmdor ovozida o'zbekcha ayting: ${text.substring(0, 500)}` }] }],
                        generationConfig: {
            ...
            try {
                const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
                    ...
                    body: JSON.stringify({
                        contents: [{ 
                            parts: [{ 
                                text: `Amir Temur tuzuklaridan davlat boshqaruvi, adolat yoki jasorat haqida bitta juda qisqa va mazmunli hikmat yozing. Faqat hikmat matnini o'zini yuboring.` 
                            }] 
                        }],
        ...
        async function getTemurAdvice(btn) {
            const input = document.getElementById('advice-input');
            const resultBox = document.getElementById('advice-result');
            const resultText = document.getElementById('advice-text');
            const span = btn.querySelector('span');
            
            if (!input.value.trim()) return;
            
            span.innerText = "✨ O'gitingiz tayyorlanmoqda...";
            btn.disabled = true;
            ...
            try {
                const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        contents: [{ parts: [{ text: `Muammo: ${input.value}. Amir Temur 'Tuzuklar'i asosida ushbu muammoga qat'iy va dono yechim taklif qiling. Matnni 'Biz shunday buyuramiz:' deb boshlang.` }] }],
                        systemInstruction: { parts: [{ text: "Siz Amir Temursiz. Vazmin va salobatli tilda javob bering." }] }
                    })
                });
            ...
            try {
                const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
                    ...
                    body: JSON.stringify({
                        ...
                        systemInstruction: { parts: [{ text: "Siz Amir Temursiz. O'zbek tilida, vazmin va donishmand ohangda javob bering. Tuzuklarga tayaning. Charon ovozi bilan gapirayotganingizni his qiling." }] }
                    })
                });
